...
if (Meteor.isServer) {
    ...
    export async function initSenseConfiguration() {
        console.log('Project root folder: ', Meteor.absolutePath)
        console.log('initSenseConfiguration: We try to set Qlik Sense variables, key things like ports and host names. ')
    ...
    //init stuff that only exists after Sense has been installed: like the certs...
    export async function initSenseServerConfiguration() {
        ...
        if (!_senseConfig.host) {
            throw new Meteor.Error('You have not started this meteor project with: meteor --settings settings-development.json ? You missed the reference to this settings file, or it is empty? Check the QRS start.bat file, and check to which settings filename it refers.');
        }
